...
<template>
  ...
  <!-- Sidebar -->
  <nav id="main-sidebar" aria-label="Navigation latérale" class="fixed inset-y-0 left-0 z-40 w-[4.5rem] bg-white dark:bg-[#0a0a0c] border-r border-gray-200/60 dark:border-zinc-800/60 transform transition-transform duration-300 ease-out md:translate-x-0 md:static md:h-full md:flex-shrink-0 flex flex-col items-center py-4 gap-1"
    ...
    <div class="flex flex-col items-center gap-1 w-full px-2 flex-1 overflow-y-auto custom-scrollbar">
      ...
      <NuxtLink to="/channels#create" class="flex items-center justify-center w-11 h-11 rounded-xl border-2 border-dashed border-gray-300 dark:border-zinc-700 text-gray-400 dark:text-zinc-600 hover:border-zinc-400 dark:hover:border-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-all duration-200" title="Créer un espace" aria-label="Créer un espace">
    ...
    <button @click="toggleTheme" class="md:hidden flex items-center justify-center w-11 h-11 rounded-xl text-gray-500 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors focus:ring-2 focus:ring-zinc-500/50 focus:outline-none" title="Thème" :aria-label="isDark ? 'Passer en mode clair' : 'Passer en mode sombre'">
      <svg v-if="isDark" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
