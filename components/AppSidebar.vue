...
<template>
  ...
  <nav id="main-sidebar" aria-label="Navigation latérale"
    ...
    <div class="flex flex-col gap-1 w-full px-2 mt-8 md:mt-0">
      <NuxtLink to="/" class="flex items-center gap-3 h-11 px-3 rounded-xl transition-all duration-200 whitespace-nowrap"
        ...
        <span class="text-sm font-medium overflow-hidden transition-opacity duration-300"
          :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">Fil d'actualité</span>
      </NuxtLink>

      <NuxtLink to="/channels" class="flex items-center gap-3 h-11 px-3 rounded-xl transition-all duration-200 whitespace-nowrap"
        ...
        <span class="text-sm font-medium overflow-hidden transition-opacity duration-300"
          :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">Explorer</span>
      </NuxtLink>

      <NuxtLink to="/profile" class="flex items-center gap-3 h-11 px-3 rounded-xl transition-all duration-200 whitespace-nowrap"
        ...
        <span class="text-sm font-medium overflow-hidden transition-opacity duration-300"
          :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">Mon profil</span>
      </NuxtLink>
    </div>
    ...
    <!-- Label "Espaces" -->
    <div class="px-5 mb-1 overflow-hidden whitespace-nowrap">
      <span class="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-zinc-600 transition-opacity duration-300"
        :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">Espaces</span>
    </div>
    ...
    <div class="flex flex-col gap-1 w-full px-2 flex-1 overflow-y-auto custom-scrollbar">
      <NuxtLink v-for="channel in channels" :key="channel.id" :to="`/channels/${channel.slug}`"
        class="flex items-center gap-3 h-11 px-3 rounded-xl transition-all duration-200 whitespace-nowrap"
        :class="$route.path === `/channels/${channel.slug}` ? 'bg-gray-900 dark:bg-white text-white dark:text-zinc-900 shadow-md' : 'text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-gray-800 dark:hover:text-zinc-200'"
        :title="channel.name"
        ...
        <span class="text-sm font-medium overflow-hidden truncate transition-opacity duration-300"
          :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">{{ channel.name }}</span>
      </NuxtLink>

      <NuxtLink to="/channels#create" class="flex items-center gap-3 h-11 px-3 rounded-xl border-2 border-dashed border-gray-300 dark:border-zinc-700 text-gray-400 dark:text-zinc-600 hover:border-zinc-400 dark:hover:border-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-all duration-200 whitespace-nowrap" title="Créer un espace" aria-label="Créer un espace">
        <svg class="w-4 h-4 shrink-0 ml-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
        </svg>
        <span class="text-sm font-medium overflow-hidden transition-opacity duration-300"
          :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">Créer un espace</span>
      </NuxtLink>
    </div>

    <!-- Bottom: theme toggle (mobile) -->
    <button @click="toggleTheme" class="md:hidden flex items-center gap-3 h-11 mx-2 px-3 rounded-xl text-gray-500 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors focus:ring-2 focus:ring-zinc-500/50 focus:outline-none whitespace-nowrap" title="Thème" :aria-label="isDark ? 'Passer en mode clair' : 'Passer en mode sombre'">
      ...
      <span class="text-sm font-medium overflow-hidden transition-opacity duration-300"
        :class="isHovered || isOpen ? 'opacity-100' : 'opacity-0'">Thème</span>
    </button>
  </nav>
</template>
