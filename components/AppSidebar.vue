    ...
    <div class="p-6 pt-16 md:pt-6 h-full overflow-y-auto custom-scrollbar">
      <div class="space-y-1">
        ...
        <NuxtLink to="/notifications"
          class="group flex items-center justify-between px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200"
          ...
          <span v-if="useNotificationsStore().unreadCount > 0"
        ...
        <button @click="toggleTheme"
          class="md:hidden group w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-xl text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-slate-200 transition-all duration-200">
          <svg v-if="isDark"
            class="mr-3 h-5 w-5 text-gray-400 dark:text-slate-500 group-hover:text-gray-600 dark:group-hover:text-slate-300"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
