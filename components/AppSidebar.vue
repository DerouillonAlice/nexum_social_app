...
<template>
  <nav class="w-64 p-6 overflow-y-auto hidden md:block border-r border-white/5 bg-[#0f111a]" style="height: 100%;">
    <div class="space-y-1">
      ...
      <a href="#"
        class="group flex items-center px-3 py-2 text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-md">
        <svg class="mr-3 h-5 w-5 text-slate-400 group-hover:text-slate-300" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="mt-8">
      <h3 class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Espaces suivis</h3>
      <div v-if="favoriteChannels.length === 0" class="px-3 text-sm text-slate-600 italic">
        Aucun espace suivi.
      </div>
      <div class="space-y-1">
        <NuxtLink v-for="channel in favoriteChannels" :key="channel.id" :to="`/channels/${channel.slug}`"
          class="group flex items-center px-3 py-2 text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-md"
          active-class="bg-blue-500/10 !text-blue-400">
          <span class="w-2 h-2 rounded-full bg-yellow-500 mr-4"></span>
          {{ channel.name }}
        </NuxtLink>

        <NuxtLink to="/channels"
          class="group flex items-center px-3 py-2 text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-md mt-4">
          <svg class="mr-3 h-5 w-5 text-slate-400 group-hover:text-slate-300" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
