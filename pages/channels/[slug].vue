    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden bg-slate-950 text-slate-200">
      ...
      <div v-if="channelError" class="flex-1 flex flex-col items-center justify-center text-center p-6">
        <div class="p-4 bg-red-500/10 border border-red-500/20 rounded-xl mb-4">
          ...
          <p class="text-slate-400 max-w-md">Impossible d'accéder au salon "<strong>{{ channelSlug }}</strong>".<br />Il
            a peut-être été supprimé ou plusieurs salons portent le même identifiant.</p>
        </div>
        <NuxtLink to="/channels"
          class="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition font-medium">
          Retour aux salons
        </NuxtLink>
      </div>
